...
/**
 * Balance History Component which shows the users all the transactions made by the group
 * @component
...
const UserBalanceHistory = () => {
  ...
  // States for filter and logged-in member's name
  ...
  // User Group ID for Logged-in Member
        ...
        try {
          ...
          for (const groupDoc of groupsSnapshot.docs) {
            ...
            if (userSnap.exists()) {
              ...
              console.log('User data from group:', userData);
  ...
  {/*Fetch transactions from the group's balances collection*/}
  useEffect(() => {
    const fetchTransactions = async () => {
      ...
      try {
        ...
        const transactions = balancesSnapshot.docs
          .map(doc => {
            const data = doc.data();
            const txName = data.memberName || data.FirstName || data.firstName || data.name || `Member ${data.userId?.substring(0, 4)}`;
            return {
              id: doc.id,
              ...data,
              memberName: txName,
          ...
          .filter(tx => {
            const normalizedTxName = tx.memberName.toLowerCase().trim().replace(/\s+/g, ' ');
            return normalizedTxName === normalizedFirstName;
          });
    ...
    fetchTransactions();
  }, [userGroupId, firstName, filterOption, filterEvent]);
            ...
            <tbody>
              {activities.length > 0 ? (
                activities.map((activity, idx) => (
                  ...
                  >
                    <td style={{ padding: '12px', textAlign: 'center', color: 'black', fontWeight: 500 }}>{activity.memberName || 'N/A'}</td>
