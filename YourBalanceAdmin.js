...
/**
 * Your Balance Admin component which allows admin to check their balance 
...
const BalanceVerification = () => {
  ...
  const fetchUserBalance = async () => {
    ...
    try {
      ...
      if (isMounted && userDocSnap.exists()) {
        ...
        setUserData(prev => ({
          ...prev,
          amount: userDataFromDB.amount || 0
        }));
      }
    } catch (error) {
      if (isMounted) {
...
}, [verified, userGroupId, userData?.uid]);
  ...
  try {
    ...
    let querySnapshot = await getDocs(query(usersRef, where('phone', '==', normalizedPhone)));
    
    if (querySnapshot.empty) {
      querySnapshot = await getDocs(query(usersRef, where('phoneNumber', '==', normalizedPhone)));
    }
